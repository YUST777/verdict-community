...
app.get('/submission', async (req, res) => {
    const url = req.query.url;
    if (!url || !url.includes('/submission/')) {
        return res.status(400).json({ error: 'Invalid submission URL' });
    }
    try {
        const scriptPath = path.join(__dirname, 'fetch_submission.js');
        const { stdout } = await execPromise(`node "${scriptPath}" "${url}"`, { timeout: 60000 });
        res.json(JSON.parse(stdout.trim()));
    } catch (error) {
        res.status(500).json({ error: 'Live fetch failed', detail: error.message });
    }
});
