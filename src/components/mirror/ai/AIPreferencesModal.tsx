...
export default function AIPreferencesModal({ isOpen, onClose, onPreferencesSaved, selectedLevel, onLevelChange, variants }: AIPreferencesModalProps) {
            ...
            (async () => {
                ...
                if (stored) {
                    try {
                        ...
                        // Decrypt API key
                        if (parsed.apiKey) {
                            const wasPlain = !isEncrypted(parsed.apiKey);
                            parsed.apiKey = await decryptValue(parsed.apiKey);

                            // Auto-migrate plain text keys to encrypted
                            if (wasPlain && parsed.apiKey) {
                                const rawStored = JSON.parse(stored);
                                rawStored.apiKey = await encryptValue(parsed.apiKey);
                                localStorage.setItem('verdict_byok_llm', JSON.stringify(rawStored));
                            }
                        }
    ...
    const handleSave = async () => {
        ...
        localStorage.setItem('verdict_byok_llm', JSON.stringify(toStore));
