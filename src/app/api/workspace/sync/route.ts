...
export async function GET(req: NextRequest) {
    try {
        ...
        const result = await query(
            'SELECT saved_code, selected_language, custom_test_cases, whiteboard_data, ai_chat_messages, ai_chat_tabs FROM user_workspaces WHERE user_id = $1 AND problem_id = $2',
            [user.id, problemId]
        );
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { problemId, savedCode, selectedLanguage, customTestCases, whiteboardData, aiChatMessages, aiChatTabs } = body;
        ...
        const queryText = `
            INSERT INTO user_workspaces (user_id, problem_id, saved_code, selected_language, custom_test_cases, whiteboard_data, ai_chat_messages, ai_chat_tabs, updated_at)
