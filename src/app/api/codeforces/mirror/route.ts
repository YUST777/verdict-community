...
export async function GET(req: NextRequest) {
        ...
        try {
            ...
            if (redisCached) {
                ...
                if (!title.includes('Bot Protection') && !title.includes('Unavailable')) {
                    console.debug('[Mirror] Redis L1 cache hit');
    ...
    if (!noCache) {
        try {
            ...
            if (cacheResult.rows.length > 0) {
                ...
                if (title.includes('Bot Protection') || title.includes('Unavailable')) {
                    console.debug('[Mirror] Cached mock detected — forcing fresh fetch');
                } else {
    ...
    const mirrorPromise: Promise<MirrorProblemData> = (async () => {
        ...
        console.debug('[Mirror] Mirror service succeeded');
        return d;
    })();
    ...
    const waybackPromise: Promise<MirrorProblemData> = (async () => {
        ...
        console.debug('[Mirror] Wayback Machine succeeded');
        return d;
    })();
...
async function scrapeDirectly(url: string) {
    ...
    try {
        console.debug('[Mirror] Strategy: Wayback Machine fetch for', url);
    ...
    try {
        ...
        console.debug('[Mirror] Strategy: mirror subdomain', mirrorUrl);
...
async function fetchViaWayback(url: string) {
    ...
    for (const timestamp of knownTimestamps) {
        try {
            ...
            console.debug('[Mirror] Fetching archived snapshot:', waybackUrl);
...
async function fetchAndParse(url: string) {
    ...
    inputs.each((i, el) => {
        const $el = $(el);
        // Better newline preservation: replace br and handle divs/paragraphs
        $el.find('br').replaceWith('\n');
        $el.find('div, p').each((_, e) => {
            $(e).append('\n');
        });
        const input = $el.text().trim();
        
        const $out = outputs.eq(i);
        $out.find('br').replaceWith('\n');
        $out.find('div, p').each((_, e) => {
            $(e).append('\n');
        });
        const output = $out.text().trim();

        testCases.push({ id: i + 1, input, output });
    });
